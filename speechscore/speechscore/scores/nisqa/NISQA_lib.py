...
class SpeechQualityDataset(Dataset):
    ...
    def _load_spec(self, audio):
        
            # Load spec    
            file_path = os.path.join(self.data_dir, self.df[self.filename_column].iloc[0])

            if self.double_ended:
                file_path_ref = os.path.join(self.data_dir, self.df[self.filename_column_ref].iloc[0])
    ...
    def __getitem__(self, index):
        ...
        if self.to_memory:
            ...
            spec = self._load_spec(self.audio)
